...
class ReadFileRequest(BaseModel):
    ...
    base_dir: str = Field(
        default=".",
        description="Directory where the file exists. should be a path",
    )
    filename: str = Field(
        ...,
        description="File name to be read will be used with the given director",
    )
...
class ReadFile(Action):
    ...
    def read_file(self, request: ReadFileRequest) -> ReadFileResponse:
        """
        Reads the contents of the file `file_name` in the directory `base_dir` and returns the contents
        if successful.
        """
        try:
